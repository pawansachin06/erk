<template>
    <div class="container">
        <div class="form-group">
            ...
            <input v-validate="'required'" type="text" class="form-control" name="post_title" placeholder="Enter title" v-model="about_data.title">
            <span class="text-danger">{{ errors.first('post_title') }}</span>
        </div>
        <div class="form-group">
            <label for="category">Description:</label>
            <vue-editor v-validate="'required'" v-model="about_data.description" name="description"> </vue-editor>
        ...
        <div style="margin-bottom: 30px;">
            <div class="d-flex justify-content-between mb-2 flex-wrap repeater-border" v-for="f in field">
                <input style="width:calc(100% - 75px);padding: 5px;" v-model="f.name" placeholder="Enter Value">
                <button style="width:75px" @click="AddField">Add Field</button>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            ...
            uploadImage: function () {
                ...
                axios.post(thisObject.site_url + '/admin/about/upload-image', formData, {
                    headers: {
                    ...
                    thisObject.about_data.image = response.data.data.url;
...
//                thisObject.errors = error.response.data.errors;
            ...
            update() {
                ...
                thisObject.$validator.validate().then(valid => {
                    if (valid) {
                        axios.post(thisObject.site_url + '/admin/about/update', {aboutdata: thisObject.about_data, fielddata: thisObject.field}).then(function (response) {
                            thisObject.$swal({
                                title: "Success",
        ...
        mounted() {
            this.field = JSON.parse(this.abouts.repeater);
            console.log(this.field);
            this.about_data = this.abouts;
            this.blog_detail.image = this.about_data.image;
        }
    }
</script>
