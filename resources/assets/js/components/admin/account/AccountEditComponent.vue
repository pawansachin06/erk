        ...
        <div class="form-group" v-if="blog_detail.image != ''">
            ...
            <div class="control_element">
                <div class="wrap-img-upload">
                    ...
                    <button class="close-img" @click.prevent='deleteImage(blog_detail.image)'><i class="fa fa-remove"></i></button>
                </div>
            </div>
        </div>
    ...
    export default {
        ...
        methods: {
            uploadImage: function () {
                ...
                axios.post(thisObject.site_url + '/admin/account/upload-image', formData, {
                    headers: {
                    ...
                    thisObject.account_data.image = response.data.data.url;
...
//                thisObject.errors = error.response.data.errors;
            ...
            deleteImage(imgUrl) {
                var thisObject = this;
                axios.post(thisObject.site_url + '/admin/account/delete-image', {image_url: imgUrl})
                        .then(function (response) {
                            thisObject.blog_detail.image = "";
                            thisObject.account_data.image = "";
                        }).catch(function (error) {
...
//            },
            update() {
                ...
                thisObject.$validator.validate().then(valid => {
                    if (valid) {
                        axios.post(thisObject.site_url + '/admin/account/update', thisObject.account_data).then(function (response) {
                            thisObject.$swal({
                                title: "Success",
        ...
        mounted() {
            this.account_data = this.accounts;
            this.blog_detail.image = this.account_data.image;
        }
    }
</script>
