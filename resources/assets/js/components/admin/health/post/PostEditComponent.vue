...
export default {
    ...
    methods: {
        loadAllCategory() {
            var thisObject = this;
            axios.get(thisObject.site_url + '/admin/health/post/loadAllCategory'
        ...
        update() {
            ...
            thisObject.$validator.validate().then(valid => {
                if (valid) {
                    axios.post(thisObject.site_url + '/admin/health/post-update', thisObject.post_data).then(function (response) {
                        thisObject.$swal({
                            title: "Success",
