<template>
    <div class="container">        
        <div class="form-group">
            ...
            <input v-validate="'required'" type="text" class="form-control" name="title" placeholder="Enter title" v-model="website_data.title">
                <span class="text-danger">{{ errors.first('title') }}</span>
        </div>
        <div class="form-group">
            <label for="category">Description:</label>
            <vue-editor v-validate="'required'" v-model="website_data.description" name="description" placeholder="Enter description"> </vue-editor>
    ...
    methods: {
        ...
        uploadImage: function () {
            ...
            axios.post(thisObject.site_url + '/admin/website_dynamics/upload-image', formData, {
                ...
                thisObject.website_data.image_url = response.data.data.url;
...
//                thisObject.errors = error.response.data.errors;
        ...
        submit() {
            ...
            thisObject.$validator.validate().then(valid => {
                if (valid) {
                    axios.post(thisObject.site_url + '/admin/website_dynamics/store', thisObject.website_data).then(function (response) {
                        thisObject.$swal({
                            title: "Success",
                            text: 'Record added successfully!',
                            icon: 'success'
                        });
                        thisObject.website_data = {                            
                            title: "",
                            description: "",
                            image_url: ""
                        };
